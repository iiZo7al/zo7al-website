...
export default async function StorePage() {
  ...
  return <main data-accent="store">
    ...
    <section className="mx-auto grid max-w-[1180px] gap-4 px-6 pb-16 sm:grid-cols-3">{([['secure',ShieldCheck],['support',Sparkles],['help',Headphones]] as const).map(([key,Icon])=><div key={key} className="flex gap-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><Icon className="mt-1 shrink-0 text-[var(--accent)]" size={23}/><div><h2 className="font-semibold">{t(key+'Title')}</h2><p className="mt-1 text-sm leading-relaxed text-[var(--text-muted)]">{t(key+'Text')}</p></div></div>)}</section>
    <section id="ranks" className="mx-auto max-w-[1600px] scroll-mt-28 px-6 pb-24"><div className="mb-10 flex flex-wrap items-end justify-between gap-4"><div><p className="text-label text-[var(--accent)]">{t("ranksEyebrow")}</p><h2 className="mt-3 text-display text-4xl">{t("ranksTitle")}</h2></div><p className="max-w-sm text-sm text-[var(--text-muted)]">{t("checkoutNote")}</p></div><StoreRanks products={catalog.products} live={catalog.live} /></section>
  </main>;
}
