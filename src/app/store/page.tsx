...
export default async function StorePage() {
  ...
  return <main data-accent="store">
    ...
    <section id="ranks" className="mx-auto max-w-[1600px] scroll-mt-28 px-6 pb-24"><Reveal className="mb-10 flex flex-wrap items-end justify-between gap-4"><div><p className="text-label text-[var(--accent)]">{t("productsTitle")}</p><h2 className="mt-3 text-display text-4xl">{t("catalogTitle")}</h2></div><p className="max-w-sm text-sm text-[var(--text-muted)]">{t("checkoutNote")}</p></Reveal><StoreRanks products={catalog.products} live={catalog.live} /></section>
  </main>;
}
