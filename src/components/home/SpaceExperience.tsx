...
interface SpaceExperienceProps { onGameStateChange?: (state: GameState) => void }
...
export default function SpaceExperience({ onGameStateChange }: SpaceExperienceProps) {
  ...
  const experience = <div ref={containerRef} data-accent="home" dir={isRtl(locale) ? "rtl" : "ltr"}
    ...
    {!active && <button type="button" onClick={triggerTransition} className="absolute bottom-28 start-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-full border border-[var(--accent)] bg-[var(--bg)]/80 px-6 py-3 text-xs font-semibold text-[var(--accent)] backdrop-blur rtl:translate-x-1/2">{t("play")} ↗</button>}
    {inFlight && !curtain && <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between gap-3 bg-gradient-to-b from-[var(--bg)] to-transparent px-5 pb-8 pt-[max(1.25rem,env(safe-area-inset-top))] sm:px-8">
    ...
    {gameState === "GAME" && !paused && !curtain && <>
      <div className="absolute inset-x-5 top-28 flex flex-wrap items-center justify-end gap-2 sm:inset-x-8">{audioControls}<button type="button" onClick={() => setPaused(true)} className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-xs">{t("pause")} · P</button></div>
      <div className="pointer-events-none absolute inset-x-4 bottom-36 text-center text-[10px] text-[var(--text-muted)]">{t("hint")}</div>
    ...
    {gameState === "GAME" && paused && !curtain && <div className="absolute inset-0 flex items-center justify-center overflow-y-auto bg-[var(--bg)]/90 p-6 text-center backdrop-blur-md"><div className="my-auto max-w-md py-6">
      <p className="mb-3 text-xs text-[var(--accent)]">{t("hold")}</p><h2 className="text-4xl font-bold">{t("takeBreath")}</h2><p className="mt-4 text-sm text-[var(--text-muted)]">{t("resumeText")}</p>
      ...
      <button onClick={() => handleGameOver(score, stars)} className="mx-auto mt-3 block rounded-full border border-[var(--border)] px-6 py-3 text-sm text-[var(--accent)]">{t("endRun")}</button><button onClick={exitGame} className="mx-auto mt-3 block p-3 text-sm text-[var(--text-muted)]">{t("exit")}</button>
    </div></div>}
    {gameState === "GAMEOVER" && <div className="absolute inset-0 overflow-y-auto bg-[var(--bg)]/90 px-4 py-6 backdrop-blur-md sm:px-6"><div className="mx-auto flex min-h-full w-full max-w-md flex-col items-center justify-center py-8 text-center">
      ...
      <GameLeaderboard key={runId} score={score} stars={stars} ticket={ticket} /><button onClick={triggerTransition} className="mt-6 w-full rounded-full bg-[var(--accent)] px-8 py-3.5 text-sm font-bold text-[var(--bg)]">{t("restart")} ↗</button><button onClick={exitGame} className="mt-2 p-3 text-xs text-[var(--text-muted)] underline underline-offset-4">{t("exit")}</button>
    </div></div>}
    <AnimatePresence>{curtain && <motion.div key="launch" initial={{ opacity: gameState === "TRANSITION" ? 0 : 1 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0.1 : 0.6 }} className="absolute inset-0 z-40 flex items-center justify-center overflow-hidden bg-[var(--bg)]" role="status">
      ...
      <motion.div initial={{ scale: reducedMotion ? 1 : 0.94 }} animate={{ scale: 1 }} className="relative max-w-lg px-6 text-center"><div className="mx-auto mb-6 h-16 w-16 rounded-full border border-[var(--accent)] shadow-[0_0_100px_var(--glow)]" /><h2 className="text-3xl font-bold sm:text-5xl">{t("launchTitle")}</h2><p className="mt-4 text-sm text-[var(--text-muted)]">{t(gameState === "TRANSITION" ? "launchText" : "loading")}</p><button onClick={exitGame} className="mt-8 px-4 py-2 text-xs text-[var(--text-muted)]">{t("exit")}</button></motion.div>
