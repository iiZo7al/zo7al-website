...
export default function SpaceExperience({ onGameStateChange }: SpaceExperienceProps) {
  ...
  const experience = <div ref={containerRef} data-space-flight={flying ? "true" : "false"} data-accent="home" dir={isRtl(locale) ? "rtl" : "ltr"}
    ...
    {gameState === "GAME" && !paused && !curtain && <>
      <div className="space-audio-controls absolute inset-x-5 top-[10rem] flex flex-wrap items-center justify-end gap-2 sm:inset-x-8">{audioControls}<button type="button" onClick={() => setPaused(true)} className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-xs">{t("pause")} · P</button></div>
